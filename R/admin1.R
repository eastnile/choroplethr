...
#' @param geoid.name The variable that identifies each
...
if (F) {
  ...
  admin1_lookup$name_lower = tolower(admin1_lookup$name_en)
  admin1_lookup$name_lower = iconv(admin1_lookup$name_lower, from = "UTF-8", to = "ASCII//TRANSLIT")
  admin1_lookup$name_lower = gsub(pattern = ' prefecture', replacement = '',  x = admin1_lookup$name_lower)
  data_prepped = left_join(df_japan_census, admin1_lookup[admin1_lookup, c('adm1_code', 'name_lower')], by = join_by(region == name_lower))
  admin1_choropleth(data_prepped, geoid.name = 'adm1_code', value.name = 'pop_2010', 
                    country_zoom = 'JPN', num_colors = 4)
