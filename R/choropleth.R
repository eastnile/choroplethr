...
Choropleth = R6Class("Choropleth", 
  ...
  public = list(
    ...
    initialize = function(user.df, geoid.name, geoid.type, value.name)
    {
      ...
      #names(user.df.prepped) = c(self$geoid.type, value.name)
      #user.df.prepped = dplyr::left_join(ref.regions, user.df[, c(geoid.name, value.name)], by = self$geoid.type)
      #user.df.prepped = user.df.prepped[!user.df.prepped[, geoid.name] %in% unmatched, ]
    ...
    render = function(projection, ncolor = NULL, user.colors = NULL, color_min = NULL, color_max = NULL) 
    {
      if (F) {
       user.colors = 5 
      }
      ...
      if (!is.null(self$ggplot_scale)) # case 1: ggplot_scale passed to function (may depreciate later)
      {
        ...
        ggscale = scale_fill_gradient(self$legend, na.value = "black", low = color_min, high = color_max)
        
      } else if (ncolor == 1) { # case 4: continuous converging scale
        ...
        mycolors = colorRampPalette(c(color_min, color_max))(self$nlevels)
        return(scale_fill_manual(values = mycolors, na.value = na.value, drop = F))       
      } else { # user variable was a factor or character but user.colors not specified
        stopifnot(class(self$user.df$value) == 'factor') 
        stopifnot(!is.null(self$nlevels))
