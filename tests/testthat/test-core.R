...
test_that("continuous data returns continous scale with num colors = 0 or 1", {
  p0 = country_choropleth(df = df_world, geoid.name = 'iso_a3', value.name = 'population', num_colors=0)
  p1 = country_choropleth(df = df_world, geoid.name = 'iso_a3', value.name = 'population', num_colors=1)
...
test_that("continuous data returns discrete scale with num colors > 1", {
  p = country_choropleth(df = df_world, geoid.name = 'iso_a3', value.name = 'population', num_colors = 7)
...
test_that("categorical data returns discrete scale and warns that num_colors ignored", {
  expect_warning(country_choropleth(df = df_world, geoid.name = 'iso_a3', value.name = 'region_type', num_colors = 7))
})

test_that("custom colors work", {
  expect_s3_class(country_choropleth(df = df_world, geoid.name = 'iso_a3', value.name = 'population', num_colors = 6,
                     custom.colors = c('grey', 'green', 'blue', 'violet', 'brown', 'orange')), 'ggplot')
  
  expect_warning(expect_s3_class(country_choropleth(df = df_world, geoid.name = 'iso_a3', value.name = 'region_type', 
                               custom.colors = c('grey', 'green', 'blue', 'violet', 'brown', 'orange')), 'ggplot'))
  
  expect_error(country_choropleth(df = df_world, geoid.name = 'iso_a3', value.name = 'population', num_colors = 3,
                               custom.colors = c('grey', 'green', 'blue', 'violet', 'brown', 'orange')))
  
  expect_warning(expect_error(country_choropleth(df = df_world, geoid.name = 'iso_a3', value.name = 'region_type', 
                               custom.colors = c('grey', 'green', 'blue'))))
})


test_that("zoom returns ggplot", {
  expect_s3_class(country_choropleth(df = df_world, geoid.name = 'iso_a3', value.name = 'population', zoom = c('USA', 'CAN', 'MEX')), 'ggplot')
})
