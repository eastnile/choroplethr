...
%\VignetteIndexEntry{f) Creating Your Own Maps}
-->

Creating Your Own Maps
===

choroplethr uses object oriented programming to support users creating choropleths with arbitrary maps. If you are interested in this functionality then I recommend you take my (now free) full-length video course [Shapefiles for R Programmers](https://ari-lamsteins-courses.thinkific.com/courses/shapefiles-for-r-programmers).

The example below assumes you have already read a shapefile into R and wish to connect it to Choroplethr. Note that this example assumes you have some familiarity with [R6](https://r6.r-lib.org/index.html), the Object-Oriented systme that choroplethr uses.
...
* Easy labeling of the title and legend with the `title` and `legend` parameters.
